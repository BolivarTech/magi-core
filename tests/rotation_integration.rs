...
/// Acceptance criteria 1 and 2, proved by OBSERVING the outcome of a real rotation rather than
/// by reading the classifier.
///
/// This is the arm that IS the fix — `ModelOutcome::MageLocal`, which inserts into
/// `state.failed_lineages` and deliberately does **not** call `register_transport_failure`.
/// Review found it had no end-to-end witness in the crate suite: every `EmptyCompletion` test
/// built without a fallback pool and therefore never reached the rotating path at all.
#[tokio::test]
async fn an_empty_completion_is_mage_local_and_leaves_the_lineage_to_the_others() {
    let caspar_primary = ScriptProvider::new("deepseek", vec![Beh::EmptyCompletion]);
    let fallback_ok = ScriptProvider::new("glm", vec![Beh::Ok]);
    let magi = MagiBuilder::new(ScriptProvider::new("m", vec![Beh::Ok]) as Arc<dyn LlmProvider>)
        .with_agent(
            AgentName::Melchior,
    ...
    // Criterion 2, read from the RUN-WIDE condemned set: the lineage Caspar gave up on is not
    // in it, so it remains available to the other two seats. `report_run_failed` derives that
    // set from the rotation kinds, which is why criterion 9 and this one are the same property
    // seen from two sides.
    let condemned = report_run_failed(&report);
    assert!(
        !condemned.contains(&Lineage::new("deepseek")),
        "an empty completion must not condemn the lineage run-wide: {condemned:?}"
    );
    assert!(
        condemned.is_empty(),
        "and nothing else was condemned either: {condemned:?}"
    );

    // Criterion 1, from the other side: the hop reports the mage-local cause by TYPE.
    let hop = &report.rotations[&AgentName::Caspar].chain[0];
    assert_eq!(hop.kind(), RotationKind::EmptyCompletion);
    assert!(hop.kind().is_mage_local());

    // And the seat recovered, which is what mage-local condemnation buys: the run is whole.
    assert!(!report.degraded);
    assert_eq!(report.agents.len(), 3);
}
