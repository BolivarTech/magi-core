...
impl Manifest {
    ...
    ///
    /// # DEFERRED: the cross has TWO directions and needs a third
    ///
    /// To be closed by MS1's fixture task, which is the first thing that makes
    /// it observable.
    ///
    /// Direction (1) asks that every manifest entry exist on disk and hash as
    /// declared; direction (2) asks that every file on disk be declared. Both
    /// start from something that IS there. **Nothing asks the other question:
    /// that a scenario which needs a fixture actually has one.** A scenario
    /// whose entry was never added, or was dropped from `wanted.txt`, produces
    /// no missing file and no orphan — the audit is clean, the preflight passes,
    /// and the scenario reports whatever it reports without its data.
    ///
    /// It is vacuous in E1: no scenario replays a fixture, which is exactly what
    /// `an_empty_manifest_verifies_clean_which_is_exactly_e1` pins. It stops
    /// being vacuous with the first scenario that needs one.
    ///
    /// Closing it needs a declaration this module does not have — which
    /// scenarios REQUIRE a fixture — and that list has to live where the
    /// scenarios do, not here, or it becomes the second hand-maintained list
    /// that disagrees with the first. `live_scenarios` is already passed in and
    /// is the natural place to carry the requirement alongside the name.
    ///
    /// # Complexity
    ///
    ...
    ///
    /// # DEFERRED: a Windows DRIVE-RELATIVE path walks past all three rules
    ///
    /// To be closed by MS1's fixture task, the first one to write real entries.
    ///
    /// `C:fixture.json` carries no separator, is not `.` and is not `..`, so
    /// this function calls it a plain filename — and on Windows it is not one.
    /// It is drive-relative, so `dir.join("C:fixture.json")` **discards `dir`**
    /// and resolves against the current directory of drive `C:`. That is the
    /// same escape the separator rule exists to stop, through a shape the rule
    /// does not name: direction (1) hashes a file from outside the corpus and
    /// direction (2), which reads one directory level, never crosses it — so
    /// the audit reports clean over a file it does not speak for. A bare `C:`
    /// does the same.
    ///
    /// It is unreachable today: the manifest is empty and every entry MS1 adds
    /// will be generated by `sync-fixtures.sh` from a filename on disk. It
    /// becomes reachable the moment an entry is written or edited by hand.
    ///
    /// Closing it means rejecting a component that is not a plain file name at
    /// all — `Path::new(path).components()` yielding anything but a single
    /// `Component::Normal` — which subsumes drive prefixes, roots, `.` and
    /// `..` in one rule instead of enumerating shapes. **`std::path` is
    /// per-platform, so the check must be written to answer the same way on
    /// both**, which is the property this function's "Both separators" section
    /// already insists on and the reason it is not a one-line change.
    ///
    /// # Parameters
    ///
...
mod tests {
    ...
    fn the_tracked_manifest_is_what_the_script_generates() {
        // DEFERRED, and this test is where it lands: it must be reworked by
        // MS1's fixture task, the first one to sync a real corpus.
        //
        // The comparison is exact and BOTH sides are header-only today. Once
        // `wanted.txt` lists fixtures, the tracked manifest carries `[[fixture]]`
        // blocks while this replica cannot produce them — the ORIGIN corpus is
        // gitignored, so `cp "$SRC/$f"` fails, `set -e` stops the script, and
        // the `out.status.success()` assertion below goes red. The failure will
        // look like a broken generator and will not be one.
        //
        // Whoever populates the corpus keeps the property — the tracked manifest
        // IS what the generator writes — while removing the dependency on an
        // origin the replica does not have: either give the replica a stub
        // corpus and a `wanted.txt` naming it, or compare only the generated
        // HEADER against the tracked file's header. The property must not be
        // dropped: it exists because the tracked manifest's header had once been
        // written by hand and diverged from its generator.
