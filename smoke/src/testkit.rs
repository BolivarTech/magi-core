...
pub fn block_comment_depth_after(line: &str, mut depth: usize) -> usize {
    let b = line.as_bytes();
    let mut i = 0;
    while i + 1 < b.len() {
...
pub fn source_declares_fn(src: &str, name: &str) -> bool {
...
/// Whether `src` EMITS `marker` — the text appears on a line that is NOT
/// commented out — as opposed to merely MENTIONING it.
///
/// # Why a substring search is not enough
///
/// A tripwire over a marker exists so a later edit cannot silence a guard
/// while the tripwire stays green. `str::contains` answers "is this text
/// written anywhere in the file", and commenting the guard out leaves the
/// text written — so the tripwire keeps passing over a guard that no longer
/// fires. That is a mechanism reporting success while guarding nothing, the
/// defect class this milestone kept producing.
///
/// # Known limitation
///
/// The scan does not lex string literals, so a `//` inside one truncates the
/// line early and a `/*` inside one raises the depth. Both push toward
/// reporting the marker ABSENT — red, loudly, on a file someone just edited —
/// which is the cheap direction. The direction that matters is a tripwire
/// staying GREEN over a marker that is gone, and neither error can produce it.
///
/// # Parameters
///
/// * `src` — the source text to inspect.
/// * `marker` — the exact text the guard is expected to emit.
///
/// # Complexity
///
/// `O(n)` in the source length.
pub fn source_emits(src: &str, marker: &str) -> bool {
    let mut depth = 0usize;
    for line in src.lines() {
        if depth == 0 {
