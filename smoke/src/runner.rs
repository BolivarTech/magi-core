    ...
    pub fn all(
        ...
        Ok(vec![
            ...
            RunSpec {
                ...
                seats: cfg.seats.clone(),
                // ONE extra candidate, built to be ineligible for two reasons at once,
                // which is the property the scenarios read.
                //
                // Derived rather than written down: it takes the FIRST seat's lineage
                // and the SECOND seat's model, so the second seat sees both that the
                // lineage is already held by another mage and that the model is one it
